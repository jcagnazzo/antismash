...
class NNResults(module_results.ModuleResults):
    """ Results for Neural Network Classification """
    def __init__(self, sequence_prediction: str, sequence_probability: float, physicochem_prediction: str, physicochem_probability: float) -> None:
        self.sequence_prediction = str(sequence_prediction)
        self.sequence_probability = float(sequence_probability)
        self.physicochem_prediction = str(physicochem_prediction)
        self.physicochem_probability = float(physicochem_probability)
...
def seq2features(seq34: str, phy34: str, s_str: str, p_str: str) -> [List, List]:
    seq_features = []
    phy_features = []
    for pos in range(0,34):
        ...
        for aa in p_str:
            if phy34[pos] == aa:
                phy_features.append(1)
            else:
                phy_features.append(0)
    return seq_features, phy_features

def extract_seq_features(queryfa: [str, str], ref: str, s_str: str, p_str: str) -> [List, List]:
    """ Extract AA codes for Neural network (NN) substrate prediction
    Arguments:
        queryfa: seq id to seq dictionary
    ...
    ## Make feature sets
    return(seq2features(seq34, phy34, s_str, p_str))


def run_neuralnet(query_fasta: Dict[str, str], extract_ref: str, seq_nn_clf: MLPClassifier, phy_nn_clf: MLPClassifier, i2s: Dict[int, str], s_str: str, p_str:str) -> NNResults:
    q_seq_features, q_phy_features = extract_seq_features(query_fasta, extract_ref, s_str, p_str)
    q_seq_features = np.array(q_seq_features).astype(np.float).reshape(1, -1)
    q_phy_features = np.array(q_phy_features).astype(np.float).reshape(1, -1)

    seq_pred = i2s[seq_nn_clf.predict(q_seq_features)[0]]
    all_seq_prob = seq_nn_clf.predict_proba(q_seq_features)
    i = 0
    for n in all_seq_prob[0]:
        if i2s[i] == seq_pred:
            seq_prob = str(n)
            break
        i += 1
        
    phy_pred = i2s[phy_nn_clf.predict(q_phy_features)[0]]
    all_phy_prob = phy_nn_clf.predict_proba(q_phy_features)
    i = 0
    for n in all_phy_prob[0]:
        if i2s[i] == phy_pred:
            phy_prob = str(n)
            break
        i += 1
    return( NNResults(seq_pred, seq_prob, phy_pred, phy_prob) )
...
def sandpuma_multithreaded(group: str, fasta: Dict[str, str], knownfaa: Dict[str, str], wildcard: str, snn_thresh: float, knownasm: str, max_depth: int, min_leaf_sup: int, ref_aln: str, ref_tree: str, ref_pkg: str, masscutoff: float, stachfa: Dict[str, str], seedfa: Dict[str, str], clf: tree.DecisionTreeClassifier, i2s: List[str], paths: List[str], pathacc: Dict[str, Dict[str, Any]], nrpsdir: str, phmmdb: str, piddb: str, seq_nn_clf: MLPClassifier, phy_nn_clf: MLPClassifier, extract_ref: str, s_str: str, p_str: str) -> SandpumaResults:
    """ SANDPUMA
    Order of processing:
    ...
    for query in fasta:
        ...
        ## NeuralNet
        #print("NeuralNet")
        #nn_result = run_neuralnet(queryfa, seq_nn_clf, phy_nn_clf)
        sp_results[query] = run_neuralnet(queryfa, extract_ref, seq_nn_clf, phy_nn_clf, i2s, s_str, p_str)
        ## PID
        #print("PID")
...
def run_sandpuma(name2seq: Dict[str, str], threads: int, knownfaa: str, wildcard: str, snn_thresh: float, knownasm: str, max_depth: int, min_leaf_sup: int, jackknife_data: str, ref_aln: str, ref_tree: str, ref_pkg: str, masscutoff:float, seed_file: str, nodemap_file: str, traceback_file: str, nrpsdir: str, phmmdb: str, piddb: str, a: float, extract_fa: str, nncodes: str):
    ...
    phy_nn_labels = []
    for name in knownfaa:
        seq_feature_matrix, phy_feature_matrix = seq2features(nn34[name]['seq'], nn34[name]['phy'], s_str, p_str)
        seq_nn_features.append(seq_feature_matrix)
        phy_nn_features.append(phy_feature_matrix)
        spec_int = allspec[name.split("_")[-1]]
        seq_nn_labels.append(spec_int)
        phy_nn_labels.append(spec_int)
    seq_nn_features = np.array(seq_nn_features).astype(np.float)
    phy_nn_features = np.array(phy_nn_features).astype(np.float)
    ## Train the neural networks
    seq_nn_clf = MLPClassifier(alpha=a)
    seq_nn_clf = seq_nn_clf.fit(seq_nn_features, seq_nn_labels)
    phy_nn_clf = MLPClassifier(alpha=a)
    phy_nn_clf = phy_nn_clf.fit(phy_nn_features, phy_nn_labels)
    ...
    args = []
    for group in groups:
         args.append([group, groups[group], knownfaa, wildcard, snn_thresh, knownasm, max_depth, min_leaf_sup, ref_aln, ref_tree, ref_pkg, masscutoff, stach_fa, seed_fa, clf, i2s, paths, pathacc, nrpsdir, phmmdb, piddb, seq_nn_clf, phy_nn_clf, extract_fa, s_str, p_str])
    return(subprocessing.parallel_function(sandpuma_multithreaded, args, cpus=threads))

def sandpuma_test(shuffle, jackknife, testfaa, trainfaa, trainaln, trainnwk, trainrefpkg, a):
    ## Set params
    ...
    extract_fa = data_dir+'A_domains_muscle.fasta'
    max_depth = 40
    min_leaf_sup = 10
    ...
    ## Actually test
    results = run_sandpuma(test_fa, threads, knownfaa, wildcard, snn_thresh, knownasm, max_depth, min_leaf_sup, jackknife_data, ref_aln, ref_tree, ref_pkg, masscutoff, seed_file, nodemap_file, traceback_file, nrpspred2basedir, phmmdb, piddb, a, extract_fa, nncodes)
    #print("\t".join(['query', 'predicat_monophyly', 'predicat_snn', 'snn_score']))
    for r in results:
        for q in r:
            l = q.split('_')
            print("\t".join([shuffle, jackknife, q,
                             l[-1], 'Sequence', str(a),
                             r[q].sequence_prediction,
                             str(r[q].sequence_probability)]))
            print("\t".join([shuffle, jackknife, q,
                             l[-1], 'Physicochem', str(a),
                             r[q].physicochem_prediction,
                             str(r[q].physicochem_probability)]))
...
sandpuma_test(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7], float(sys.argv[8]))



#p_str = 'ENUHGCP-'
#s_str = 'RHKDESTNQAVILMFYWGCP-'
#allfa = fasta.read_fasta(sys.argv[1])
#ref = os.path.dirname(os.path.realpath(sys.argv[0]))+'/data/A_domains_muscle.fasta'
#for name in allfa:
#    seq = allfa[name]
#    seq.replace("-", "")
#    extract_seq_features({name: seq}, ref, s_str, p_str)
